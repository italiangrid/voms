...
public class PKIVerifier {
     ...
     *
     * @param vomsStore the VOMSTrustStore object which represents the
     * vomsdir store.
     * @param caStore the PKIStore object which represents the CA store.
     */
     ...
     * vomsdir store.
     * 
     * @throws IOException if there have been IO errors.
     * @throws CertificateException if there have been problems parsing a
     * certificate
     * @throws CRLException if there have been problems parsing a CRL.
     */
    public PKIVerifier(VOMSTrustStore vomsStore) throws IOException, CertificateException, CRLException {
        this(vomsStore, 
             new PKIStore("/etc/grid-security/certificates", PKIStore.TYPE_CADIR, true));
    }

    /**
     * Initializes the verifier.  The CA store is initialized at:
     * "/etc/grid-security/certificates.", while the VOMS store is
     * initialized at "/etc/grid-security/vomsdir".
     *
     * @throws IOException if there have been IO errors.
     * @throws CertificateException if there have been problems parsing a
     * certificate
    ...
    public PKIVerifier() throws IOException, CertificateException, CRLException {
        this(new PKIStore("/etc/grid-security/vomsdir", PKIStore.TYPE_VOMSDIR, true),
             new PKIStore("/etc/grid-security/certificates", PKIStore.TYPE_CADIR, true));
    }
     ...
     *
     * This method MUST be called prior to disposal of this object, 
     * otherwise memory leaks and runaway threads will occur.
     */
    public void cleanup() {
        if (vomsStore != null)
     ...
     *
     * @param store the new VOMS store.
     */
     ...
     *
     * @param ac the Attribute Certificate to verify.
     *
     * @return true if the attribute certificate is verified,
     * false otherwise.
     */
    public boolean verify(AttributeCertificate ac) {
        ...
        if (lsc != null) {
            ...
            while (!success && vecIndex < dns.size()) {
                Vector dnList = (Vector)dns.get(vecIndex);
                List certs = certList.getCerts();
                ...
                while (dnIter.hasNext() && certIter.hasNext() && !doBreak) {
                    String dn = null;
                    String is = null;
                    try {
                        dn = (String)dnIter.next();
                        is = (String)dnIter.next();
                    }
                    ...
                    if (!dn.equals(candidateDN) ||
                        !is.equals(candidateIs))
                        doBreak = true;
                }

                if (!doBreak && !dnIter.hasNext() && !certIter.hasNext())
                    success = true;
            }
            if (success == true) {
        ...
        if (certificates == null) {
            ...
            if (logger.isDebugEnabled())
                logger.debug("Looking for hash: " + PKIUtils.getHash(ac.getIssuer()) + " for certificate: " + ac.getIssuer().getName());
        ...
        if (certificates == null) {
            logger.error("Cannot find certificates for AC");
            return false;
        }
     ...
     *
     * @param certs the chain to verify.
     *
    ...
    public boolean verify(X509Certificate[] certs) {
        ...
        logger.info("Certificate verification: Verifying certificate '" + certs[0].getSubjectDN().getName() +"'");
        ...
        for (int i = 1; i < certs.length; i++) {
            ...
            if (PKIUtils.checkIssued(certs[i], certs[i-1])) {
                ...
                if (logger.isDebugEnabled())
                    logger.debug("ELEMENT: " + currentCert.getSubjectDN().getName());
            }
        ...
        if (logger.isDebugEnabled()) {
            ...
            while (j.hasNext())
                logger.debug("Content: " + ((X509Certificate)j.next()).getSubjectDN().getName());
        }
        ...
        if (PKIUtils.selfIssued(currentCert)) {
            ...
            if (candidates != null && (index = candidates.indexOf(currentCert)) != -1) {
                ...
                if (logger.isDebugEnabled())
                    logger.debug("ELEMENT: " + candidate.getSubjectDN().getName());
            }
            else {
                logger.error("Certificate verification: self-signed certificate '"+ candidate.getSubjectDN().getName() + "' not found among trusted certificates.");
                return false;
            }
        }
        else {
            ...
            do {
                ...
                if (candidates != null) {
                    ...
                    while (i.hasNext()) {
                        ...
                        if (logger.isDebugEnabled())
                            logger.debug("Candidate = " + candidate.getSubjectDN().getName());
        ...
        while (!certStack.isEmpty()) {
            ...
            if (logger.isDebugEnabled())
                logger.debug("VERIFYING : " + currentCert.getSubjectDN().getName());

            if (PKIUtils.selfIssued(currentCert)) {
                if (currentLength != 0) {
                    logger.error("Certificate verification: Self signed certificate not trust anchor");
                    logger.error("subject: " + currentCert.getSubjectDN().getName());
                    success = false;
                    break;
                }
                else {
            ...
            try {
                ...
                logger.error("Certificate verification: certificate in chain expired. " + e.getMessage(), e);
                logger.error("Faulty certificate: " + currentCert.getSubjectDN().getName());
                logger.error("End validity      : " + currentCert.getNotAfter().toString());
                return false;
            }
            catch (CertificateNotYetValidException e) {
                logger.error("Certificate verification: certificate in chain not yet valid. " + e.getMessage(), e);
                logger.error("Faulty certificate: " + currentCert.getSubjectDN().getName());
                logger.error("Start validity      : " + currentCert.getNotBefore().toString());
                return false;
            }
    ...
    private boolean allowsPath(X509Certificate cert, X509Certificate issuer) {
        Hashtable signings = caStore.getCRLs();
        //        System.out.println("CLASS IS: " + signings.get(PKIUtils.getHash(issuer)).getClass());
                    ...
                    if (signCandidate.getAccessIDCA().equals(issuerSubj)) {
                        ...
                        while (subjIter.hasNext()) {
                            ...
                            if (certSubj.matches(subj)) {
                                matched = true;
                                logger.info("Subject: '" + certSubj + "' matches with subject: '" + subj + "' from signing policy.");
                            }
                            logger.info("Subject: '" + certSubj + "' does not match subject: '" + subj + "' from signing policy.");
                        }
                ...
                if (candidateCRL != null) {
                    ...
                    }
                    {
                        if (candidateCRL.getCriticalExtensionOIDs() == null) {
                            if (candidateCRL.getIssuerX500Principal().equals(issuer.getIssuerX500Principal())) {
                                if (candidateCRL.getNextUpdate().compareTo(new Date()) >= 0 &&
                                    candidateCRL.getThisUpdate().compareTo(new Date()) <= 0) {
                                    
                                    X509CRLEntry entry = candidateCRL.getRevokedCertificate(cert.getSerialNumber());
                                    if (entry == null) {
                                        return false;
                                    }
                                }
